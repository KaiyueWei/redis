...
// | nstr| len | str1|
// +-----+-----+----+
static uint32_t parse_req(const uint8_t *data, size_t size, std::vector<std::string> &out){
...
static void make_response(const Response &resp, Buffer *buf){
	uint32_t resp_len = 4 + (uint32_t)resp.data.size();
	buf_append(buf, (const uint8_t *)&resp_len, 4);
	buf_append(buf, (const uint8_t*)&resp.status, 4);
	buf_append(buf, resp.data.data(), resp.data.size());
}
